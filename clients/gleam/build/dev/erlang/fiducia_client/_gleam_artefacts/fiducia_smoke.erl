-module(fiducia_smoke).
-compile([no_auto_import, nowarn_unused_vars, nowarn_unused_function, nowarn_nomatch, inline]).
-define(FILEPATH, "src/fiducia_smoke.gleam").
-export([main/0]).

-file("src/fiducia_smoke.gleam", 5).
-spec main() -> {ok, gleam@dynamic:dynamic_()} |
    {error, fiducia:fiducia_error()}.
main() ->
    C = fiducia:new(<<"http://127.0.0.1:8799/"/utf8>>),
    echo(fiducia:health(C), nil, 9),
    echo(fiducia:status(C), nil, 11),
    echo(fiducia:kv_get(C, <<"does/not exist"/utf8>>), nil, 13),
    Meta = gleam@json:object(
        [{<<"region"/utf8>>, gleam@json:string(<<"us-east-1"/utf8>>)}]
    ),
    echo(
        fiducia:service_register(
            C,
            <<"api"/utf8>>,
            <<"i-1"/utf8>>,
            <<"10.0.0.1:9000"/utf8>>,
    ...
    echo(
        fiducia:lock_acquire(
            C,
            <<"orders/checkout"/utf8>>,
            {some, <<"worker-a"/utf8>>},
            {some, 30000},
            false
        ),
    ...
    echo(fiducia:lock_acquire(C, <<"k2"/utf8>>, none, none, true), nil, 22),
    Dead = fiducia:new(<<"http://127.0.0.1:9"/utf8>>),
    echo(fiducia:health(Dead), nil, 26).
