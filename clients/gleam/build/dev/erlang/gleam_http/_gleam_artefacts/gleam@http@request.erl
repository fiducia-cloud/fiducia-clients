...
-spec set_header(request(EHQ), binary(), binary()) -> request(EHQ).
...
-spec prepend_header(request(EHT), binary(), binary()) -> request(EHT).
...
-spec set_body(request(any()), EHY) -> request(EHY).
set_body(Req, Body) ->
    {request,
...
-spec map(request(EIA), fun((EIA) -> EIC)) -> request(EIC).
...
-spec set_query(request(EIM), list({binary(), binary()})) -> request(EIM).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(EIQ), gleam@http:method()) -> request(EIQ).
...
-spec set_scheme(request(EIX), gleam@http:scheme()) -> request(EIX).
...
-spec set_host(request(EJA), binary()) -> request(EJA).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(EJD), integer()) -> request(EJD).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(EJG), binary()) -> request(EJG).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(EJJ), binary(), binary()) -> request(EJJ).
...
-spec remove_cookie(request(EJP), binary()) -> request(EJP).
