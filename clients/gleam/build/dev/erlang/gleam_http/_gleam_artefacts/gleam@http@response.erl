...
-type response(ERK) :: {response, integer(), list({binary(), binary()}), ERK}.

-file("src/gleam/http/response.gleam", 87).
?DOC(" Set the body of the response, overwriting any existing body.\n").
-spec set_body(response(any()), ESH) -> response(ESH).
...
-spec try_map(response(ERL), fun((ERL) -> {ok, ERN} | {error, ERO})) -> {ok,
        response(ERN)} |
    {error, ERO}.
...
-spec set_header(response(ERZ), binary(), binary()) -> response(ERZ).
...
-spec prepend_header(response(ESC), binary(), binary()) -> response(ESC).
...
-spec map(response(ESJ), fun((ESJ) -> ESL)) -> response(ESL).
...
-spec expire_cookie(response(ESU), binary(), gleam@http@cookie:attributes()) -> response(ESU).
