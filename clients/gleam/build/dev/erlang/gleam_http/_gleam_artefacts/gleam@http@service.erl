...
-spec map_response_body(
    fun((EVG) -> gleam@http@response:response(EVH)),
    fun((EVH) -> EVJ)
) -> fun((EVG) -> gleam@http@response:response(EVJ)).
...
-spec prepend_response_header(
    fun((EVL) -> gleam@http@response:response(EVM)),
    binary(),
    binary()
) -> fun((EVL) -> gleam@http@response:response(EVM)).
...
-spec ensure_post(gleam@http@request:request(EVP)) -> {ok,
        gleam@http@request:request(EVP)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(EVY)) -> EWA)) -> fun((gleam@http@request:request(EVY)) -> EWA).
