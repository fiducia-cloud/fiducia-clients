...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(FCJ)) -> {ok, FCJ} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(FCD)) -> {ok, FCD} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(FBZ)) -> {ok, FBZ} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(FCP), fun((FCP) -> json())) -> json().
...
-spec array(list(FCT), fun((FCT) -> json())) -> json().
