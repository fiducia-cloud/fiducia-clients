...
export class FiduciaClient {
  ...
  private retryable(err: unknown, signal: AbortSignal | undefined, method: string, idempotencyKey?: string): boolean {
    ...
    if (err instanceof FiduciaError) return [408, 425, 429, 500, 502, 504].includes(err.status);
  ...
  async counterAdd(key: string, delta: number, opts: RequestControlOpts & { prevRevision?: number } = {}): Promise<any> {
    return this.request("POST", `/v1/counters/add`, { key: key, delta: delta, prev_revision: opts.prevRevision }, opts);
  }

  async counterSet(key: string, value: number, opts: RequestControlOpts & { prevRevision?: number } = {}): Promise<any> {
    return this.request("POST", `/v1/counters/set`, { key: key, value: value, prev_revision: opts.prevRevision }, opts);
  }
  ...
  async barrierCreate(name: string, policy: Record<string, any>, opts: RequestControlOpts & { expected?: number; deadlineMs?: number } = {}): Promise<any> {
    return this.request("POST", `/v1/barriers/create`, { name: name, policy: policy, expected: opts.expected, deadline_ms: opts.deadlineMs }, opts);
  }

  async barrierArrive(name: string, participant: string, opts: RequestControlOpts & { weight?: number; veto?: boolean } = {}): Promise<any> {
    return this.request("POST", `/v1/barriers/arrive`, { name: name, participant: participant, weight: opts.weight, veto: opts.veto }, opts);
  }
  ...
  async taskCreate(name: string, taskType: string, opts: RequestControlOpts & { payload?: Record<string, any>; deadlineMs?: number } = {}): Promise<any> {
    return this.request("POST", `/v1/tasks/create`, { name: name, task_type: taskType, payload: opts.payload, deadline_ms: opts.deadlineMs }, opts);
  }

  async taskClaim(name: string, worker: string, opts: RequestControlOpts & { ttlMs?: number } = {}): Promise<any> {
    return this.request("POST", `/v1/tasks/claim`, { name: name, worker: worker, ttl_ms: opts.ttlMs }, opts);
  }

  async taskProgress(name: string, worker: string, fencingToken: number, opts: RequestControlOpts & { percent?: number; checkpoint?: Record<string, any> } = {}): Promise<any> {
    return this.request("POST", `/v1/tasks/progress`, { name: name, worker: worker, fencing_token: fencingToken, percent: opts.percent, checkpoint: opts.checkpoint }, opts);
  }

  async taskComplete(name: string, worker: string, fencingToken: number, opts: RequestControlOpts & { result?: Record<string, any> } = {}): Promise<any> {
    return this.request("POST", `/v1/tasks/complete`, { name: name, worker: worker, fencing_token: fencingToken, result: opts.result }, opts);
  }

  async taskFail(name: string, worker: string, fencingToken: number, opts: RequestControlOpts & { retryable?: boolean } = {}): Promise<any> {
    return this.request("POST", `/v1/tasks/fail`, { name: name, worker: worker, fencing_token: fencingToken, retryable: opts.retryable }, opts);
  }

  async taskCancel(name: string, opts: RequestControlOpts = {}): Promise<any> {
    return this.request("POST", `/v1/tasks/cancel`, { name: name }, opts);
  }
  ...
  async effectPrepare(name: string, effectType: string, idempotencyKey: string, opts: RequestControlOpts & { payload?: Record<string, any>; risk?: string; requiredApprovals?: number } = {}): Promise<any> {
    return this.request("POST", `/v1/effects/prepare`, { name: name, effect_type: effectType, payload: opts.payload, risk: opts.risk, idempotency_key: idempotencyKey, required_approvals: opts.requiredApprovals }, opts);
  }

  async effectApprove(name: string, principal: string, opts: RequestControlOpts = {}): Promise<any> {
    return this.request("POST", `/v1/effects/approve`, { name: name, principal: principal }, opts);
  }

  async effectCommit(name: string, opts: RequestControlOpts & { result?: Record<string, any> } = {}): Promise<any> {
    return this.request("POST", `/v1/effects/commit`, { name: name, result: opts.result }, opts);
  }

  async effectAbort(name: string, opts: RequestControlOpts = {}): Promise<any> {
    return this.request("POST", `/v1/effects/abort`, { name: name }, opts);
  }
  ...
  async handoffOffer(name: string, resource: string, from: string, to: string, fromToken: number, opts: RequestControlOpts & { context?: Record<string, any>; ttlMs?: number } = {}): Promise<any> {
    return this.request("POST", `/v1/handoffs/offer`, { name: name, resource: resource, from: from, to: to, from_token: fromToken, context: opts.context, ttl_ms: opts.ttlMs }, opts);
  }

  async handoffAccept(name: string, to: string, opts: RequestControlOpts = {}): Promise<any> {
    return this.request("POST", `/v1/handoffs/accept`, { name: name, to: to }, opts);
  }

  async handoffReject(name: string, to: string, opts: RequestControlOpts = {}): Promise<any> {
    return this.request("POST", `/v1/handoffs/reject`, { name: name, to: to }, opts);
  }
  ...
  async decisionPropose(name: string, question: string, options: Record<string, any>, policy: Record<string, any>, opts: RequestControlOpts & { deadlineMs?: number } = {}): Promise<any> {
    return this.request("POST", `/v1/decisions/propose`, { name: name, question: question, options: options, policy: policy, deadline_ms: opts.deadlineMs }, opts);
  }

  async decisionVote(name: string, voter: string, opts: RequestControlOpts & { option?: string; confidence?: number; weight?: number; veto?: boolean; evidence?: Record<string, any> } = {}): Promise<any> {
    return this.request("POST", `/v1/decisions/vote`, { name: name, voter: voter, option: opts.option, confidence: opts.confidence, weight: opts.weight, veto: opts.veto, evidence: opts.evidence }, opts);
  }
  ...
  async budgetSet(name: string, limit: Record<string, any>, opts: RequestControlOpts = {}): Promise<any> {
    return this.request("POST", `/v1/budgets/set`, { name: name, limit: limit }, opts);
  }

  async budgetReserve(name: string, reservationId: string, holder: string, amount: Record<string, any>, opts: RequestControlOpts = {}): Promise<any> {
    return this.request("POST", `/v1/budgets/reserve`, { name: name, reservation_id: reservationId, holder: holder, amount: amount }, opts);
  }

  async budgetCommit(name: string, reservationId: string, actual: Record<string, any>, opts: RequestControlOpts = {}): Promise<any> {
    return this.request("POST", `/v1/budgets/commit`, { name: name, reservation_id: reservationId, actual: actual }, opts);
  }

  async budgetRelease(name: string, reservationId: string, opts: RequestControlOpts = {}): Promise<any> {
    return this.request("POST", `/v1/budgets/release`, { name: name, reservation_id: reservationId }, opts);
  }
  ...
  async claimAssert(name: string, subject: string, predicate: string, author: string, opts: RequestControlOpts & { value?: Record<string, any>; confidence?: number; evidence?: Record<string, any>; validUntilMs?: number } = {}): Promise<any> {
    return this.request("POST", `/v1/claims/assert`, { name: name, subject: subject, predicate: predicate, value: opts.value, confidence: opts.confidence, author: author, evidence: opts.evidence, valid_until_ms: opts.validUntilMs }, opts);
  }

  async claimSupport(name: string, agent: string, opts: RequestControlOpts = {}): Promise<any> {
    return this.request("POST", `/v1/claims/support`, { name: name, agent: agent }, opts);
  }

  async claimContest(name: string, agent: string, opts: RequestControlOpts & { reason?: string } = {}): Promise<any> {
    return this.request("POST", `/v1/claims/contest`, { name: name, agent: agent, reason: opts.reason }, opts);
  }

  async claimResolve(name: string, accepted: boolean, opts: RequestControlOpts = {}): Promise<any> {
    return this.request("POST", `/v1/claims/resolve`, { name: name, accepted: accepted }, opts);
  }

  async claimSupersede(name: string, supersededBy: string, opts: RequestControlOpts = {}): Promise<any> {
    return this.request("POST", `/v1/claims/supersede`, { name: name, superseded_by: supersededBy }, opts);
  }
