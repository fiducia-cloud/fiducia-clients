...
The first hard-gated tier (`ts`, `python`, `go`, `rust`) is generated from
reviewed templates plus `operations.json`; most other clients are generated as
thin wrappers. Run `python3 generate.py --check` to detect drift. Each language
folder owns a `publish.sh` release entrypoint — see
