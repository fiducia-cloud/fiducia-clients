...
- `browser-e2e.yml` — transpiles the TypeScript client with the locked compiler
  and executes its browser contract in an exact Chrome for Testing release.
  The live loopback server exercises real browser fetch redirect behavior,
  AbortController timeouts, Web Crypto holder generation, marked not-leader
  retries with a stable idempotency key, and SSE stream decoding.
...
concurrency control, and a job timeout. Browser CI also pins the Chrome for
Testing version and runs against a loopback-only server with a restrictive CSP,
no persistent browser profile, and bounded request bodies. Workflows validate
