...
#
# ESLint is treated as a globally installed tool rather than a per-repository
# dependency. Nothing here installs packages or reaches the network.
...
[ "${ORES_LINT_SKIP_JS}" = "1" ] && {
  echo "ores-lint[js]: skipped (ORES_LINT_SKIP_JS=1)"
  exit 0
}
...
if ESLINT=$(find_local_eslint "$ROOT"); then
  ...
  ESLINT_KIND="global (PATH)"
elif [ -n "$GLOBAL_ROOT" ] && [ -x "$GLOBAL_ROOT/.bin/eslint" ]; then
  ESLINT="$GLOBAL_ROOT/.bin/eslint"
  ESLINT_KIND="global"
else
...
CONFIG=""
for candidate in eslint.config.mjs eslint.config.js eslint.config.cjs; do
  [ -f "$ROOT/$candidate" ] && {
...
[ -z "$CONFIG" ] && {
  echo "ores-lint[js]: no flat eslint config found - skipping"
  exit 0
}
...
(
  cd "$ROOT" && "$ESLINT" . \
    ...
    --format "$DIR/eslint/formatter.mjs"
) >"$OUT" 2>&1 || RC=$?
